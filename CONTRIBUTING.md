...
When running parallel worker agents, limit the number of parallel agents to 3 unless told otehrwise, letting them work on batches. You should use a `WORKING.md` file in the project directory to coordinate such work. Tracking a work summary, testing details if any as well as the immediate task list, list of agents as they start, progress and finish a given task. This should allow for relatively easy continuation of broken work, or incomplete work due to a session window limit. Ask agents to list their batch tasks and progress on ./temp/agent-X.md while working and remove the file when done.

The files `AGENTS.md` and `CLAUDE.md` should be exact duplicates of eachother. Changes to one should be made to both.
...
- **Errata convention (2.30 only, for now):** `version/2.30/ripscrip/` is the one tree with a real draft-vs-shipped gap - its canonical text is a pre-release ALPHA 4 draft, not a final spec. For this tree, the chapter pages state the corrected, as-shipped reading an implementer should follow, with no inline qualification; draft-vs-shipped divergences and corrected draft errors are recorded instead in [`version/2.30/ripscrip/ERRATA.md`](version/2.30/ripscrip/ERRATA.md), grouped by chapter with the draft's text, the shipped reality and its evidence, and a pointer back to the page section it explains. This does not relax the rule above - corrections still need to be clearly marked and cited - it just moves where the marking lives for this tree. 1.54 shipped a final spec and needs no such split; 3.1 is an avowed reconstruction whose inline evidence tags are integral to the page, not a historical aside to file elsewhere.
