...
[◀ Prev: Buttons](12-buttons.md) · [Contents](README.md) · [Next: Host Commands & Control Characters ▶](14-host-commands.md)
...
The `<text>` parameter for this command is also segmented in nature, much like the [RIP_BUTTON](12-buttons.md#rip_button) command is. An example of a segmented `<text>` parameter for the RIP_DEFINE command might be as follows:
...
The Query Text Variable RIPscrip command instructs the terminal to immediately respond with some piece of information, whether statically stored (i.e., in a database), stored internally in RAM (dynamic information), or pre-defined [Text Variables](15-text-variables.md).
...
NOTE: This command is very flexible in that you can specify control characters, [pick-list definitions](17-popup-lists.md), Text Variables, and [Host Command template](18-host-command-templates.md) definitions. See the section entitled [HOST COMMANDS](14-host-commands.md) for a more detailed explanation of these features.
