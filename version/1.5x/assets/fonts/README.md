...
Font numbers 0–10 (`00`–`0A` in the protocol's 2-digit "meganum" text encoding) are exactly as tabulated in the 1.54 specification's [RIP_FONT_STYLE command](../../ripscrip/06-text-output.md#rip_font_style). Font 0 is the only bit-mapped entry; fonts 1–10 are scalable stroked fonts served by the `.CHR` files above. The stroked and bitmap font formats are fully documented in this repository's techspecs: [BGI stroked fonts (`.CHR`)](../../techspecs/bgi-stroked-fonts.md) and [RIPterm bitmap fonts (`RIPTERM.FNT`)](../../techspecs/bitmap-fonts.md).
