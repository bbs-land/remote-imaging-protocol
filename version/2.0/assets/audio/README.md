# Audio - RIPterm 2.x Distributions
...
On the wire, RIPscrip 2.2+ plays background WAV audio via [RIP_PLAY_AUDIO](../../../3.0/ripscrip/4.5-audio-files.md#rip_play_audio) (specified since 2.0 ALPHA 3) and the [`$)FILE.WAV$` playback prefix](../../../3.0/ripscrip/5.3-local-playback.md). RIPtel likewise had **no dedicated audio directory**: per the recovered help text, local playback searches the host directory first, then `ICONS\` - audio files lived alongside the icons, and the RIPtel install enables audio via `AUDIO=TRUE` in `RIPscrip.ini`. See the [research notes](../../../3.0/research/riptel-help-extraction.md) for the audio-related strings recovered from the RIPtel binaries.
