...
> **Editor's note — shipped form:** The shipping engine does not use this 13-character argument layout. RIPterm Professional 2.0 (January 1995) already drove `y` with a **26-fixed-character block** followed by the font name — its bundled demo script SHADOW.RIP issues `!|y00000X02020000001a1a000000marin` — the same layout later observed throughout the RIPtel 3.0 corpus (see the [3.x reconstruction](../../3.x/ripscrip/10-level-0-commands-s-z.md#rip_extended_font_style)). The predefined font names of this draft (COURIER, HELV, …) also never shipped: RIPterm 2.0's outline fonts were the Atech FastFont families COBB, DEFAULT, DIXON, MARIN, and SYMBOL, distributed as `.FF1` files (see the [2.x font assets](../assets/fonts/README.md)).
