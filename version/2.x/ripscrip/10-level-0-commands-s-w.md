...
**Format:** `!|b <base_math>` **Example:** `!|b1S`
...
_[Editor's note: the ALPHA 4 draft assigns level-0 command `b` to both this command and [RIP_EXTENDED_TEXT_WINDOW](08-level-0-commands-a-f.md#rip_extended_text_window) — an unresolved collision in the draft. The shipping RIPscrip 3.0 driver resolved it by moving RIP_SET_BASE_MATH to level-0 **`J`** (so the format above became `!|J <base_math>`, e.g. `!|J10` for MegaNums), leaving `b` to the extended text window. This is confirmed by the RIPtel 3.1 demo corpus, where `J10` opens 90 of 116 TeleGrafix-authored scripts and is annotated "Set base math to MegaNums (base 36)" in TeleGrafix's own comments. See the [3.x entry](../../3.x/ripscrip/09-level-0-commands-g-r.md#rip_set_base_math).]_
...
**Format:** `!|D <num> <bits> <start> <c1> ... <cn>` **Example:** `!|D030180GhZPzr3aZr3`
...
The viewport is physically bound to the current [drawing port](02-drawing-ports.md). In other words when this command is executed, it will modify the viewport for the current drawing port. This in effect, modifies the clipping rectangle for that port. If any of the viewport coordinates would extend beyond the boundaries of the drawing port, they will be adjusted to fit inside the port. _(v2.A3)_
...
To re-activate a viewport that was previously deactivated, either send a correctly configured RIP_SET_VIEWPORT command, or issue a query command with a `$AVP$` [text variable](17-text-variables-general.md) (Activate ViewPort). _(v2.A4)_
