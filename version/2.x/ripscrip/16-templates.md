...
[◀ Prev: Local File Playback & Pop-Up Lists](15-local-playback-popup-lists.md) · [Contents](README.md) · [Next: Text Variables: General, Date/Time & Sound ▶](17-text-variables-general.md)

## Templates

A template is a special variety of a host command that is used to construct other host commands. They are only used with the [RIP_BUTTON](11-level-1-commands.md#rip_button) command, not with `RIP_MOUSE` or [RIP_QUERY](11-level-1-commands.md#rip_query). Their use is solely dedicated to the Button command. In fact, their usefulness is predominantly designed for Radio Buttons and Check Box Buttons but this doesn't mean that you cannot use them for other purposes.
