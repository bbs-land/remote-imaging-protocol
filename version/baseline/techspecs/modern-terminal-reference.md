...
This page describes what a _modern_ BBS terminal implements on the text side, using SyncTERM (C, `~/src/rip-tools/sbbs/src/syncterm/` + the cterm core in `~/src/rip-tools/sbbs/src/conio/`) and icy_term (Rust, `~/src/rip-tools/icy_tools/`) as reference points. **Everything here is a MODERN capability description.** For each feature the table below states explicitly whether any evidence for it exists in the RIPterm/RIPtel materials examined on the [previous page](ansi-vt-support.md) — and for most of these features the answer is **no**. A future implementer should treat this page as "what users of a RIP-capable terminal will expect in 2026", and the previous page as "what the historical products actually did"; the two must not be conflated.
...
| 256-color / 24-bit SGR (`38;5`, `38;2`, `CSI … t`) | Yes | Yes | **None at the ANSI layer.** 1.54 Appendix B documents only classic attribute SGR. RIPterm 2.x's 256-color/direct-RGB support is RIPscrip-command-level (SVGA palette commands — see [2.x techspecs](../../2.30/techspecs/README.md)), not an SGR extension |
