...
36 entries, each a text window definition: corner coordinates (in text coordinates), MicroANSI font number, current ANSI attributes, cursor position and visibility, vertical scrolling margins, and activation state. The window's _contents_ are not part of the definition. The default window is full-screen 80×25. A deactivated current window discards incoming raw text. See [Color, Audio & Text Windows](06-color-audio-text.md) for the extended (resolution-independent) text window added in this era.
