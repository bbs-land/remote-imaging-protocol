...
`4L,3D`–`91,9K` = (165, 121)–(325, 344) in the standard 1280×960 world frame; column `0` of stream `1`; flags `0001`. Text drawn into the region uses the current font (`y` / `Y`) and colors, and is word-wrapped to the region's width.
...
N2_TITLE.RIP pairs `1T` with the 1.54 `1t` (RIP_REGION_TEXT) command instead of raw lines — `1t1…` lines carry justified text, with `1t0…` on each paragraph's final line (justify flag off), confirming the flowed text is fully justified within its column.
...
!|J10|n2000|M08|fZKQO                               Standard 3.0 prologue: base 36, 2-byte
