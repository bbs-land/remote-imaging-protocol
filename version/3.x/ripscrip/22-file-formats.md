...
_Evidence: corpus (ICONS/_.BMH byte comparison — see [binary triage §3](../research/riptel-binary-formats.md)).*
...
_Evidence: WP (§3.7.2, GrayPEG); HLP (jpeg_ strings; readme progressive-display note); corpus (N2_PHOTO.RIP, SPECLEFX.RIP).*

## `.RFF` Outline Fonts — Atech FastFont

The scalable [outline-font system](10-level-0-commands-s-z.md) is powered by a **licensed third-party engine: Atech Software's "FastFont"** (of Atech's AllType font-conversion product). Every shipped `.RFF` ends with the trailer `COPR:` + "Copyright 1991, Atech Software, Carlsbad CA", and leaked buffer garbage inside the files names `*.FF1` (Atech's own extension) — `.RFF` is evidently "RIPscrip FastFont", renamed. This is now confirmed directly: RIPterm Professional 2.0 (January 1995) shipped five of the eight families (COBB, DEFAULT, DIXON, MARIN, SYMBOL) as literal `.FF1` files, and the `.FF1` → `.RFF` reformat itself shipped with **RIPterm 2.30** (October 1997), whose five `.RFF` files are byte-identical to RIPtel's — both generations preserved in the [2.x font assets](../../2.x/assets/fonts/README.md). Key structural facts (byte-accurate layout in [binary triage §1](../research/riptel-binary-formats.md)):
...
_Evidence: corpus (FONTS/RIPscrip.maf — [binary triage §1](../research/riptel-binary-formats.md)); HLP (MicroANSI font-loading error strings; MESSAGES.HLP text-mode list)._
...
_Evidence: corpus (116-file census — [script census](../research/riptel-script-census.md))._
