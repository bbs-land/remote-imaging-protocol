...
| 2.0 / 2.1 / 2.2 | 1995–1997 | The undocumented "RIPscrip-2" product era: the engine as shipped in RIPterm Professional 2.0 (Jan 1995), 2.20.00 (Nov 19, 1995), 2.20.01 (Nov 28, 1995), and 2.3 (Oct 25, 1997), with **no specification published after ALPHA 4** (no product labeled "2.1" has surfaced). The shipping engine moved past the draft from day one — RIPterm Pro 2.0 already used the 26-character RIP_EXTENDED_FONT_STYLE argument block (SHADOW.RIP) rather than ALPHA 4's 13-character form, and v2.20.01 added the icon-transparency flag (`<flags2>` bit 128) to RIP_BUTTON_STYLE, absent from the ALPHA 4 table. Features often credited to "3.x" were already shipping in the January 1995 2.0 release — background **.WAV audio playback** (specified since ALPHA 3; HMI sound drivers), JPEG image display, SVGA/256-color modes, BMP icons, and Atech `.FF1` outline fonts (see the [2.x assets](../../2.x/assets/fonts/README.md)). See [Color, Audio & Text Windows](06-color-audio-text.md#background-wav-playback). |
