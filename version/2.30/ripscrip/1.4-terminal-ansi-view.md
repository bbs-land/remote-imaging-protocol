...
This page covers the text side of the 2.x display model: [text windows](../../GLOSSARY.md) — the virtual TTY regions that display all non-RIPscrip (ASCII/ANSI) data — and the commands that define, switch, position within, and erase them. Text-window content is [CP437](../../GLOSSARY.md) text rendered from the client's own MicroANSI fonts (byte-level font formats are in the [MicroANSI font techspec](../techspecs/3.3-microansi-fonts.md)). Where 1.54 had a single text window ([1.4 in the v1.54 reference](../../1.54/ripscrip/1.4-terminal-ansi-view.md)), 2.x allows up to 36 text-window definitions in the text window [data table](7.1-data-tables.md#text-window-table), one current at a time, each with its own ANSI attributes, cursor, and activation state — and adds the resolution-independent [RIP_EXTENDED_TEXT_WINDOW](#rip_extended_text_window). Text windows support ANSI and VT-102 screen control; the terminal-emulation baseline is the [ANSI/VT support reference](../../baseline/techspecs/ansi-vt-support.md). Chapter-5 text variables ($ATW$, $DTW$, $ETW$, $TWGOTO$, and the rest of the [text-window variable family](5.1-text-variables.md#text-window-related-text-variables)) manipulate these windows from host commands.
...
_[Editor's note: the ALPHA 4 draft assigns `b` to both this command and [RIP_SET_BASE_MATH](1.2-math-and-coordinates.md#rip_set_base_math). The shipping RIPscrip 3.0 driver kept `b` for the extended text window and moved RIP_SET_BASE_MATH to `J` — see the note on that entry.]_
...
In RIPscrip, you are allowed to have up to 36 separate [text windows](1.4-terminal-ansi-view.md) on screen simultaneously (0-9, A-Z). These windows maintain their own "current ANSI attributes", cursor position, text window dimensions/location, cursor ON/OFF status and whether the window is actived or deactivated.
