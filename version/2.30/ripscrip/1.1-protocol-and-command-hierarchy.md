...
This page covers the wire-level shape of the RIPscrip 2.x language: the `!|` line form, the `|` delimiter, the expanded multi-level [command level](../../GLOSSARY.md) hierarchy, escaping and quoting, line continuation, comments and command grouping, the [auto-sense](../../GLOSSARY.md) handshake, and the protocol-level commands that open and close a scene ([RIP_HEADER](#rip_header), [RIP_NO_MORE](#rip_no_more)). It also explains how each command entry in this reference is documented. The number systems ([MegaNums](../../GLOSSARY.md), [UltraNums](../../GLOSSARY.md), [Base Math](../../GLOSSARY.md)) used by command parameters are in [1.2 Math & Coordinates](1.2-math-and-coordinates.md); file-authoring conventions are in [6.0 Writing 2.x .RIP Files](6.0-writing-rip-files.md). The 1.54 record of this material is [1.1 in the v1.54 reference](../../1.54/ripscrip/1.1-protocol-and-command-hierarchy.md); the 2.x syntax is a superset — rules 10 (parameter shortening), 12 (SOH/STX mid-line starts), 14 (quoting host-command characters), 15 (comments) and 16 (corrupt-command recovery) are the 2.x additions.
...
> **Editor's note:** Every shipping 2.x-generation RIPterm binary — Professional 2.0, 2.20.01, and 2.30 — carries the same literal auto-sense response string `RIPSCRIP020000`: version 02.00 with the trailing vendor/sub-version digits zeroed (RIPTERM.EXE strings from all three recovered installs; see the provenance table in [CONTRIBUTING.md](../../../CONTRIBUTING.md)). The reported language level never advanced past 2.00.00 through the end of the RIPterm line in October 1997, even as the product versions did; RIPterm 2.30's documentation likewise states it "supports the RIPscrip 2.0 language fully" (RIPTERM.FAQ v1.7).
...
The chapters of this reference detail the RIPscrip command set (the consolidated index is [9.0 Command Reference](9.0-command-reference.md)). Each command has these aspects:
