...
This page covers the wire commands that exchange data and files with the host: querying [text variables](5.1-text-variables.md) ([RIP_QUERY](#rip_query)), the obsolete variable-define command, file queries, timing control for local playback, block-mode file transfer, and the UU-encoded data blocks new to 2.x. It also carries capability negotiation — the `$IFS$`/`$TERMINFO$` mechanism by which a host discovers what a terminal supports — and the multi-lingual groundwork. The delivery workflow built from these commands (where files land, the extension routing table) is described creator-side in [6.2 Asset Delivery & Storage](6.2-asset-delivery-and-storage.md). The 1.54 record is [5.5 in the v1.54 reference](../../1.54/ripscrip/5.5-queries-and-advanced-commands.md); the 2.x changes are RIP_QUERY's per-window deferred modes, two new protocols (Zmodem without crash recovery _(v2.A0)_, Super Kermit _(v2.A4)_), revised block-mode file types, [RIP_DELAY](#rip_delay)/[RIP_BAUD_EMULATION](#rip_baud_emulation) at level 3, and [RIP_BEGIN_UUENCODE_BLOCK](#rip_begin_uuencode_block) at level 9. (RIP_SCROLL — formerly RIP_COPY_REGION — moved to [4.0 Icons & Bitmaps](4.0-icons-and-bitmaps.md#rip_scroll).)
...
_[Editor's note: the v2.A3 revision notes announce a `RIP_FILE_DELETE` command "to remove unneeded JPEG or WAVE files", but no command entry for it exists anywhere in the ALPHA 4 draft. File deletion survives as the [`$FILEDEL$`](5.1-text-variables.md#filedel) text variable.]_
