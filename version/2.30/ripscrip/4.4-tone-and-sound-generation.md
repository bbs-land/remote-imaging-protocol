...
This page covers speaker tone generation — waveform-style sounds produced by the terminal itself, triggered through Active [Text Variables](5.1-text-variables.md) anywhere text variables are allowed. This is the 1.54 sound capability ([4.3 in the v1.54 reference](../../1.54/ripscrip/4.3-tone-and-sound-generation.md)) carried forward and extended: the canned effects now take parameters (`$BEEP(freq,ms)$`, `$MUSIC(count)$`, `$PHASER(start,stop,inc,time)$`, …), and the new chainable [`$T$`](#t) tone variable _(v2.A4)_ plays a single tone with **no trailing pause**, so multiple `$T$` variables strung together produce continuous multi-frequency sequences — simple music. Each variable's definition includes the exact `sound()`/`delay()` recipe so every terminal produces the same effect. Digitized audio **files** (`.WAV`) are a separate, new 2.x capability — see [4.5 Audio Files](4.5-audio-files.md). A terminal that does not implement a particular sound should still recognize the variable and do nothing (see the [implementer note in 5.1](5.1-text-variables.md)).
