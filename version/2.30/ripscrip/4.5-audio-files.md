...
This page covers digitized audio files — new in the 2.x generation, with no 1.54 counterpart. The format is Microsoft **WAVE** (`.WAV`) PCM audio, played in the background while graphics continue to draw. Playback is triggered from the command stream with [RIP_PLAY_AUDIO](#rip_play_audio), or at the host-command layer with the `$)FILE.WAV$` playback prefix ([5.3 Local File Playback](5.3-local-playback.md#local-audio-file-playback)); the special filename `$OFF$` stops a running sound. This is distinct from the terminal's own speaker-tone generation ([4.4 Tone & Sound Generation](4.4-tone-and-sound-generation.md)), which needs no files at all. Byte-level RIFF/PCM layout and playback semantics are in the [audio techspec](../techspecs/3.4-audio.md).
