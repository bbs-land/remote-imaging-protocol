...
This page covers templates — the [host-command](5.0-host-commands.md) construction system used with [buttons](4.3-buttons.md), and above all with radio-button and check-box groups. Templates are defined and executed inside a button's host-command block; up to 36 may be active at once, one per button group. The 1.54 record is [5.2 in the v1.54 reference](../../1.54/ripscrip/5.2-host-command-templates.md); the mechanism is carried forward essentially unchanged, with 2.x host-command text variables (including parameterized forms) usable throughout. Which host-command features may appear where is summarized in [5.0 Host Commands](5.0-host-commands.md#host-commands-directives---what-can-go-where).
