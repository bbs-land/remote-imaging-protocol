...
This page covers the closed shapes — rectangles, bars, rounded rectangles, polygons — their filled variants, and the fill-style state (twelve stock patterns plus user-defined 8×8 patterns). Filled interiors use the current fill color and fill pattern over the current background color; outlines use the drawing color and line style from [2.0](2.0-color-modes-and-palettes.md) and [2.1](2.1-lines.md). Two things changed from 1.54: borders on filled objects can now be disabled ([RIP_SET_BORDER](#rip_set_border), on by default for backward compatibility), and filled regions follow the resolution-independent fill rule — interiors fill _between_ coordinate lines, one pixel short of the right/bottom border — explained in [1.2 Math & Coordinates](1.2-math-and-coordinates.md#the-mathematics-of-graphics-and-coordinates).

**Flood fill is gone.** [RIP_FILL](#rip_fill) — the 1.54 flood-fill command — was removed from the language in 2.x; clients parse and ignore it. Its entry below carries the removal rationale and the spec's table of filled-object replacements. For what the removed command actually did, see [2.3 in the v1.54 reference](../../1.54/ripscrip/2.3-shapes-and-fills.md#rip_fill).
...
This command operates similarly to the [RIP_BAR](#rip_bar) command when borders are disabled, however the lower-left and lower-right edges of the filled rectangle obey the resolution independent method of performing fills (see the section called ["THE MATHEMATICS OF GRAPHICS AND COORDINATES"](1.2-math-and-coordinates.md) for details on how this works. _(v2.A3)_
