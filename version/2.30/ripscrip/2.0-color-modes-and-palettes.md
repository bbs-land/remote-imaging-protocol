...
This page covers color in RIPscrip 2.x: how the 1.54 16-color Desktop Palette generalizes to a 256-entry Drawing Palette (color lookup table), the two color modes — palette mapping and direct RGB encoding — and the palette commands old and new. It also carries the [write mode](../../GLOSSARY.md) command, whose raster operations (COPY/XOR/OR/AND/NOT) now apply to **all** primitives, not just simple lines as in 1.54. Colors marked `:CM` in command entries follow the current color mode set by [RIP_SET_COLOR_MODE](#rip_set_color_mode); direct-RGB values are always four-digit [UltraNums](../../GLOSSARY.md) regardless of the Base Math setting. Up to 36 complete palettes live in the color palette [data table](7.1-data-tables.md#color-palette-table), switched with [RIP_SWITCH_PALETTE](#rip_switch_palette). The 16-color 1.54 model this extends is [2.0 in the v1.54 reference](../../1.54/ripscrip/2.0-colors-and-attributes.md); renderer-side palette/DAC scaling details are in the [canvas, palette & RGB techspec](../techspecs/2.0-canvas-palette-rgb.md).
...
This command determines whether Color Mapping mode is in use, or if Direct RGB Colors are being used for color codes. See [the beginning of this specification](2.0-color-modes-and-palettes.md) for a detailed explanation of the differences between these modes.
