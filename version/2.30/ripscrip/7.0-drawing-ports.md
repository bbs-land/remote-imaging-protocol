...
This page covers [drawing ports](../../GLOSSARY.md) in full — the fundamental 2.x drawing abstraction, new in this generation. A port is an area where graphics can be drawn: **screen ports** are regions of the visible display (port #0 is always the full screen and cannot be redefined), **offscreen bitmap ports** ("clipboard ports") are drawing surfaces the user cannot see until their contents are copied to a screen port. Up to 36 ports may be defined; exactly one is current at any moment and receives all drawing operations. Each port has its own coordinate origin and its own [viewport](../../GLOSSARY.md) (clipping rectangle). Concept-level orientation is in [1.3 World View & Virtual Canvas](1.3-world-view-virtual-canvas.md); ports live in the drawing port [data table](7.1-data-tables.md#drawing-port-table) and can be saved/restored through [data backup areas](7.2-data-backup-areas.md); the 1.54-era clipboard commands that now ride on top of ports are in [4.0 Icons & Bitmaps](4.0-icons-and-bitmaps.md).
