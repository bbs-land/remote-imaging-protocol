...
This page covers text drawn into the graphics environment: positioning the drawing cursor, the text commands, the classic font system (one bitmap font plus ten scalable stroked fonts, now with justification and dropshadow flags), the new extended outline-font system, and the text-region commands carried forward from 1.54 for formatted text blocks. This is graphics text — distinct from the [text window](1.4-terminal-ansi-view.md)'s TTY output. The 1.54 record is [3.0](../../1.54/ripscrip/3.0-text-output-and-fonts.md) and [3.1](../../1.54/ripscrip/3.1-text-regions.md) in the v1.54 reference; the 2.x changes are the `<flags>` justification/dropshadow parameter of [RIP_FONT_STYLE](#rip_font_style), the magnification-factor table replacing simple ×N scaling for vector fonts, and [RIP_EXTENDED_FONT_STYLE](#rip_extended_font_style) for scalable outline fonts. Per-font metric tables (top/bow/base/drop) are with the button-label material in [4.3 Buttons](4.3-buttons.md#font-metrics-and-descenders), where the specification uses them. Font file formats are implementer material: [FastFont outline fonts](../techspecs/3.2-fastfont-fonts.md) and [MicroANSI terminal fonts](../techspecs/3.3-microansi-fonts.md).
...
> **Editor's note — shipped form:** The shipping engine does not use this 13-character argument layout. RIPterm Professional 2.0 (January 1995) already drove `y` with a **26-fixed-character block** followed by the font name — its bundled demo script SHADOW.RIP issues `!|y00000X02020000001a1a000000marin` — the same layout later observed throughout the RIPtel corpus (see the [3.x reconstruction](../../3.1/ripscrip/3.0-text-output-and-fonts.md#rip_extended_font_style)). The predefined font names of this draft (COURIER, HELV, …) also never shipped: RIPterm 2.0's outline fonts were the Atech FastFont families COBB, DEFAULT, DIXON, MARIN, and SYMBOL, distributed as `.FF1` files (see the [2.x-generation font assets](../../2.30/assets/fonts/README.md)).
