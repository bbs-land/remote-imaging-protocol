...
This page introduces the [host command](../../GLOSSARY.md) "action language" — the text a terminal sends back to the host (or acts on locally) when the user interacts with [mouse fields](4.2-mouse-fields-and-pointer.md), [buttons](4.3-buttons.md), or [queries](5.5-file-transfer-and-queries.md#rip_query) — together with the control-character notation used inside it, the summary of which host-command features may appear where, the host-defined screen-refresh sequence, and the buffer-preservation care implementers must take. The 1.54 record is [5.0 in the v1.54 reference](../../1.54/ripscrip/5.0-host-commands.md).

With Mouse regions, Buttons and Text Variable Query ability, you can control the Terminal/Paint programs and how they react with the BBS in many ways. To accomplish this, there are several features of RIPscrip that permit you to do special actions based on different circumstances — in effect, an "action language". Among the various abilities are:

- Control-Character specification (this page)
- Pre-defined Text Variables & User-defined text variables — [5.1 Text Variables](5.1-text-variables.md)
- Pop-up pick-lists — [5.4 Pop-Up Lists](5.4-popup-lists.md)
- Query text variable contents (pre-defined & user variables) — [5.1](5.1-text-variables.md#querying-text-variables) and [RIP_QUERY](5.5-file-transfer-and-queries.md#rip_query)
- Local file playback directives (`$>`, `$<`, `$(`, `$)`) — [5.3 Local File Playback](5.3-local-playback.md)
- A Host Command "Template" system for added intelligence — [5.2 Templates](5.2-templates.md)
