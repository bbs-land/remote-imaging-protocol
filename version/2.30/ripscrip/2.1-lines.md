...
This page covers the straight-line drawing primitives — single pixels, lines, and multi-segment poly-lines — together with the line-style state (pattern and thickness) they draw with. All of these draw in the current drawing color ([RIP_COLOR](2.0-color-modes-and-palettes.md#rip_color)), honor the current [write mode](2.0-color-modes-and-palettes.md#rip_write_mode), and are clipped to the current port's [viewport](../../GLOSSARY.md). Two 2.x extensions over [the 1.54 record](../../1.54/ripscrip/2.1-lines.md): patterned lines can draw their "off" pixels in the background color ([RIP_BACK_COLOR](2.0-color-modes-and-palettes.md#rip_back_color)) instead of transparently, and line thickness is now arbitrary (1.54 allowed only 1 or 3 pixels). Curved outlines are in [2.2 Curves](2.2-curves.md); closed and filled shapes are in [2.3 Shapes & Fills](2.3-shapes-and-fills.md).
