...
This page covers raster imagery on the terminal: capturing screen regions to a [clipboard](../../GLOSSARY.md) port, pasting them back with logical put modes, loading and writing disk-based [icons](../../GLOSSARY.md), and the block-move commands. Under 2.x icons are Windows **BMP** bitmaps — `.BMP` for icons, `.BMH` for [hot icons](../../GLOSSARY.md), `.BMM` for [icon masks](../../GLOSSARY.md) — replacing the 1.54 `.ICN`/`.HIC`/`.MSK` formats (roles in [6.1 Content File Roles](6.1-content-file-roles.md); byte-level format in the [icon formats techspec](../techspecs/3.0-icon-formats.md)). The 1.54 clipboard is likewise generalized: it is now simply the first offscreen [drawing port](7.0-drawing-ports.md), located through a **clipboard pointer** the commands below maintain — the 1.54-era commands keep working, and new commands ([RIP_LOAD_BITMAP](#rip_load_bitmap), [RIP_COPY_BLIT](#rip_copy_blit), the [port-copy family](7.0-drawing-ports.md#rip_port_copy)) add scaling, dithering, transparency and arbitrary port targeting. The 1.54 record is [4.0 in the v1.54 reference](../../1.54/ripscrip/4.0-images-and-icons.md). A host command can also stamp a bitmap via the `$<file.BMP$` playback prefix — see [5.3 Local File Playback](5.3-local-playback.md).
