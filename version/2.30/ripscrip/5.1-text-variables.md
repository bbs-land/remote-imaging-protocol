...
This page covers the 2.x [text variable](../../GLOSSARY.md) system — greatly expanded from [5.1 in the v1.54 reference](../../1.54/ripscrip/5.1-text-variables.md). Text variables are `$NAME$` sequences usable in [host commands](5.0-host-commands.md), [queries](5.5-file-transfer-and-queries.md#rip_query), and keystroke macros; **data** variables substitute a value, **active** variables perform an action on the terminal. The 2.x additions are parameters — `$SAVE(8)$` instead of `$SAVE8$` — with a formal syntax-description notation, plus whole new families covering ports/viewports, extended text windows, the environment, data tables and backup areas, capability negotiation, and resets. Sound-generation variables are documented in [4.4 Tone & Sound Generation](4.4-tone-and-sound-generation.md); the playback prefixes (`$>`, `$<`, `$(`, `$)`) are in [5.3 Local File Playback](5.3-local-playback.md); the template forms (`$?$`, `$?x$`) are in [5.2 Templates](5.2-templates.md). The consolidated table of every variable is [9.1 Text Variable Reference](9.1-text-variable-reference.md).
