...
This page covers the graphics side of the 2.x display model at the level a scene author needs: the [world coordinate](../../GLOSSARY.md) frame as the logical drawing space, the [viewport](../../GLOSSARY.md) that maps and clips drawing onto the [screen](../../GLOSSARY.md), and the reset semantics that return the environment to a known state. Under 2.x every drawing operation happens inside a [drawing port](../../GLOSSARY.md) — the screen is simply port #0 — and each port carries its own viewport; this page treats that model at concept level, while the full port machinery (defining, switching, copying between ports) is chapter 7's subject, starting at [7.0 Drawing Ports](7.0-drawing-ports.md). The world→device coordinate translation math is in [1.2 Math & Coordinates](1.2-math-and-coordinates.md#world-coordinate-systems). The 1.54 model this generalizes — a single fixed 640×350 graphics window with a clipping viewport — is [1.3 in the v1.54 reference](../../1.54/ripscrip/1.3-world-view-virtual-canvas.md).

The concept in brief: you draw in world coordinates; the world frame (default 640×350 for backward compatibility, settable up to 65535×65535) defines the logical space; the terminal maps world coordinates onto whatever physical resolution it runs at; and the current port's viewport defines both the clipping rectangle and the origin for drawing. A 1.54 scene runs unchanged because the defaults reproduce the 1.54 environment exactly.
...
_[Editor's note: the `RIP_SET_DRAWING_FRAME` command referred to in the v2.A0 paragraph above was dropped before the ALPHA 4 draft — no entry for it exists anywhere in the specification. In practice the viewport's 1:1 world-coordinate mapping described above is the only drawing-frame behavior defined.]_
