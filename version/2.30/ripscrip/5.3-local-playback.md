...
This page covers the local-playback mechanism: the family of prefix text variables that replay files already on the terminal's disk from within any [host command](5.0-host-commands.md) or query. 1.54 had only `$>file$` for `.RIP` scenes ([5.3 in the v1.54 reference](../../1.54/ripscrip/5.3-local-playback.md)); 2.x extends the idea across every media type, each with its own prefix character: `$>` for RIPscrip scenes, `$<` for BMP bitmaps, `$(` for compressed images (JPEG), and `$)` for audio files _(v2.A1)_. All of them resolve filenames connection-directory-first ([6.2 Asset Delivery & Storage](6.2-asset-delivery-and-storage.md#lookup-order-file-override)). This is the _trigger-at-the-host-command-layer_ path — each media type also has a dedicated command-stream command, compared explicitly [below](#playback-prefixes-vs-dedicated-commands).
