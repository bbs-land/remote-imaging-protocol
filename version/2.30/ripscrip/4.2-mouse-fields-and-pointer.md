...
This page covers [mouse fields](../../GLOSSARY.md) — host-defined rectangular hot regions that transmit a [host command](../../GLOSSARY.md) when clicked — the commands that delete them, and control of the mouse pointer shape (new in 2.x). Styled [buttons](4.3-buttons.md) build on the same click-to-host-command machinery. What the transmitted text can contain is chapter 5's subject, starting at [5.0 Host Commands](5.0-host-commands.md). The 1.54 record is [4.1 in the v1.54 reference](../../1.54/ripscrip/4.1-mouse-fields.md); 2.x adds world-coordinate placement, the region-selective [RIP_KILL_ENCLOSED_MOUSE_FIELDS](#rip_kill_enclosed_mouse_fields), and [RIP_SET_MOUSE_CURSOR](#rip_set_mouse_cursor).
