...
- **10 Borland BGI stroked fonts (`.CHR`)** - the classic RIPscrip 1.54 vector font set, selected on the wire by [`Y` RIP_FONT_STYLE](../../ripscrip/3.0-text-output-and-fonts.md#rip_font_style) (font IDs 01-0A).
- **8 Atech FastFont outline fonts (`.RFF`)** - the RIPscrip 3.0 scalable outline-font system, selected by name via [`y` RIP_EXTENDED_FONT_STYLE](../../ripscrip/3.0-text-output-and-fonts.md#rip_extended_font_style); each file exposes 10 style variants (see below).
...
Format internals are documented byte-level in the [binary-format triage](../../research/riptel-binary-formats.md); recovered help text on the font systems is in the [help extraction](../../research/riptel-help-extraction.md); the condensed reference pages are in [File Formats](../../ripscrip/6.1-content-file-roles.md). The definitive format techspecs are the 2.x [FastFont](../../../2.30/techspecs/3.2-fastfont-fonts.md) and [MicroANSI](../../../2.30/techspecs/3.3-microansi-fonts.md) pages plus the 3.x deltas in [`version/3.1/techspecs/`](../../techspecs/README.md) ([RFF additions](../../techspecs/3.2-fastfont-additions.md), [RIPscrip.maf](../../techspecs/3.3-microansi-maf-delta.md)).
