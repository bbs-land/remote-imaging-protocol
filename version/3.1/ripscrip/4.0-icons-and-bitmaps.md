...
This page covers raster imagery on the terminal: capturing screen regions to a [clipboard](../../GLOSSARY.md) port, pasting them back with logical put modes, loading and writing disk-based [icons](../../GLOSSARY.md), and the block-move commands. Under the 2.30 engine that 3.x inherits, icons are Windows **BMP** bitmaps — `.BMP` for icons, `.BMH` for [hot icons](../../GLOSSARY.md), `.BMM` for [icon masks](../../GLOSSARY.md) — replacing the 1.54 `.ICN`/`.HIC`/`.MSK` formats (roles in [6.1 Content File Roles](6.1-content-file-roles.md); byte-level format in the [v2.30 icon formats techspec](../../2.30/techspecs/3.0-icon-formats.md), unchanged in 3.x, with the RIPtel file census in the [binary-formats research](../../3.1/research/riptel-binary-formats.md)). The 1.54 clipboard is likewise generalized: it is simply the first offscreen [drawing port](7.0-drawing-ports.md), located through a **clipboard pointer** the commands below maintain — the 1.54-era commands keep working, and the 2.30 additions ([RIP_LOAD_BITMAP](#rip_load_bitmap), [RIP_COPY_BLIT](#rip_copy_blit), the [port-copy family](7.0-drawing-ports.md#rip_port_copy)) add scaling, dithering, transparency and arbitrary port targeting. The RIPtel corpus shows the shipped practice: [RIP_LOAD_BITMAP](#rip_load_bitmap) is the workhorse (backgrounds, button skins, transparent-color composites), the 1.54-lineage [RIP_LOAD_ICON](#rip_load_icon) survives chiefly for backward compatibility with on-the-fly `.ICN`→BMP conversion ([below](#legacy-icn-icons-and-on-the-fly-conversion)), and for port-addressed copies the demos overwhelmingly prefer the level-2 [RIP_PORT_COPY](7.0-drawing-ports.md#rip_port_copy). The 1.54 record is [4.0 in the v1.54 reference](../../1.54/ripscrip/4.0-images-and-icons.md); the 2.30 record is [4.0 in the v2.30 reference](../../2.30/ripscrip/4.0-icons-and-bitmaps.md). A host command can also stamp a bitmap via the `$<file.BMP$` playback prefix — see [5.3 Local File Playback](5.3-local-playback.md#local-bitmap-playback--filebmp).
...
**Attributes used:** Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_get_image)) · HLP (`RIP_GetImage`; clipboard port concept, `$SCB$`/`$RCB$`/`$P$` variables) · corpus (4 uses in LANDSCPE.RIP) · SyncTERM (ripper.c:15847)
...
In older versions of RIPscrip (v1.54 and earlier), there was only the screen and one offscreen bitmap port (the clipboard). Under the 2.30/3.x engine, you may have multiple offscreen bitmap ports (or clipboards). There are three commands from RIPscrip 1.54 that pertain directly to the concept of a clipboard (RIP_GET_IMAGE, [RIP_PUT_IMAGE](#rip_put_image) and [RIP_WRITE_ICON](#rip_write_icon)). To facilitate the integration of these commands in the newer architecture, a clipboard pointer is used. The clipboard pointer is created when the RIP_GET_IMAGE command is used. When this command is executed, the software will scan from port #1 to port #35 (port #0 is skipped because it's the screen). It searches for the first open (unused) port. Once found, the clipboard pointer is set to point to this port. This allows the RIP_PUT_IMAGE and RIP_WRITE_ICON command to know which port number is associated with the clipboard. _(v2.A3)_
...
**Attributes used:** Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_put_image)) · HLP (`RIP_PutImage`; `$P$` paste-clipboard variable) · corpus (1 use in NEWSPAPR.RIP) · SyncTERM (ripper.c:16481)
...
**Attributes used:** Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_write_icon)) · HLP (`RIP_WriteIcon`) · SyncTERM (ripper.c:16969) — not observed in the RIPtel demo corpus
...
**Attributes used:** Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_load_icon)) · HLP (`RIP_LoadIcon`; plus 1.54-compat strings "Can't convert RIP 1.54 Icon %s to 2.0 BMP format!") · SyncTERM (ripper.c:16302) — not observed in the RIPtel demo corpus
...
**Attributes used:** Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_load_bitmap)) · HLP (`RIP_LoadBitmap`; also the `$<FILE.BMP$` macro shorthand) · corpus (69 uses in 28 files) — not in SyncTERM
...
**Attributes used:** Draw Color, Back Color, Fill Style, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_copy_blit)) · HLP (`RIP_CopyBlit`) · corpus (88 uses in 6 files) — not in SyncTERM
...
**Attributes used:** Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/4.0-icons-and-bitmaps.md#rip_scroll)) · HLP (`RIP_Scroll`) · SyncTERM (ripper.c:16246 — **documentation-only stub**: full 1.54 spec comment, then a bare `break;`) — not observed in the RIPtel demo corpus
...
The 1.54 technique of stamping a mask icon in AND mode and the image in OR mode still works with the put modes above, and the `.BMM` mask extension exists for that role (see [6.1 Content File Roles](6.1-content-file-roles.md#file-roles) — no `.BMM` files are known to have shipped, and the 3.x record adds no sighting). The preferred route since 2.30 is [RIP_LOAD_BITMAP](#rip_load_bitmap)'s transparency flag (flag 8 with `<trans_color>`), which makes one color of the bitmap show through to the graphics behind it in a single operation — exactly how the corpus composites its torch and décor art (DBACK.FN).
