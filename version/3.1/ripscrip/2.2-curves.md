...
This page covers the curved outline primitives: circles, ellipses (ovals), circular and elliptical arcs, and the bezier family — including the 2.30 poly-bezier additions that chain line and curve segments into one object — plus the 3.x era's **skewed-oval family**: six symbol-opcode primitives (`&`, `-`, `]`, `[`, `+`, `_`) named in TeleGrafix's own NEWCMDS.RIP demo comments that exist in no published specification and draw ellipses rotated to arbitrary angles. All draw in the current drawing color and honor line thickness; the circle/oval/arc family does not honor line patterns, while the bezier family does. Arc angles are in degrees, 0 at the 3 o'clock position, increasing counterclockwise to 360. These primitives honor XOR and the other [write modes](2.0-color-modes-and-palettes.md#rip_write_mode) (a 1.54 limitation lifted in 2.30). The filled counterparts (filled circle/oval, pie slices, filled poly-bezier, poly-polygons) are in [2.3 Shapes & Fills](2.3-shapes-and-fills.md); the filled members of the skewed-oval family stay on this page with their outline siblings, as the 3.x record groups them. The 1.54 record of the base commands is [2.2 in the v1.54 reference](../../1.54/ripscrip/2.2-curves.md); the 2.30 record is [2.2 in the v2.30 reference](../../2.30/ripscrip/2.2-curves.md).
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_circle)) · HLP (`RIP_Circle`) · SyncTERM (ripper.c:13546) — not observed in the RIPtel demo corpus
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_oval)) · corpus (6 uses in 4 files) · SyncTERM (ripper.c:13850) — not in the RIPSCRIP.HLP name inventory (only `RIP_OvalArc`)
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_arc)) · HLP (`RIP_Arc`) · SyncTERM (ripper.c:13470) — not observed in the RIPtel demo corpus
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_oval_arc)) · HLP (`RIP_OvalArc`) · corpus (2 uses in SHAPES.RIP) · SyncTERM (ripper.c:13848 — falls through to `O`)
...
_Evidence: corpus (NEWCMDS.RIP, SHAPES.RIP); see the [script census](../../3.1/research/riptel-script-census.md)._

The six commands below exist in no published specification — not 1.54, not 2.00a4. They were recovered from the RIPtel demo corpus, where NEWCMDS.RIP demonstrates each new primitive over a coordinate grid with TeleGrafix's own naming comments ("Show RIP_SKEWED_OVAL", "Show a RIP_SKEWED_OVAL_PIE_SLICE", …), making the canonical names verbatim TeleGrafix. They add what the 1.54/2.30 [RIP_OVAL](#rip_oval) family cannot do: ellipses whose axes are **rotated to an arbitrary angle**. All field meanings come from census analysis; the rotation unit is presumed degrees _(hypothesis)_. The filled members honor [RIP_SET_BORDER](2.3-shapes-and-fills.md#rip_set_border) like the documented filled primitives — NEWCMDS.RIP demos each "With a border" and "Without a border". None of these commands appear in the RIPSCRIP.HLP name inventory or in SyncTERM.
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_bezier)) · HLP (`RIP_Bezier`) · corpus (22 uses in 4 files) · SyncTERM (ripper.c:14140)
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_poly_bezier)) · corpus (2 uses in SHAPES.RIP) · SyncTERM (ripper.c:14870 — shared parser, closed/unfilled flags) — absent from the RIPSCRIP.HLP name inventory, though its poly-bezier error strings apply
...
**Attributes used:** Draw Color, Back Color, Line Style, Write Mode, Viewport, Port, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.2-curves.md#rip_poly_bezier_line)) · corpus (12 uses in ONLINE.RIP, CURVES.RIP, SHAPES.RIP) · SyncTERM (ripper.c:14721 — shared bezier parser, open/unfilled flags) — poly-bezier commands are absent from the RIPSCRIP.HLP name inventory, though its error strings validate them ("The last segment of a poly-bezier cannot be a curve")
