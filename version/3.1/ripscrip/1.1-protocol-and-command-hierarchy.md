...
This page covers the wire-level shape of the RIPscrip 3.x language: the `!|` line form, the `|` delimiter, the multi-level [command level](../../GLOSSARY.md) hierarchy, escaping and quoting, line continuation, comments and command grouping, the [auto-sense](../../GLOSSARY.md) handshake and its 3.x-era `RIPSCRIP03000…` replies, the scene header segments the 3.0 driver formalizes, and the protocol-level commands that open and close a scene ([RIP_HEADER](#rip_header), [RIP_NO_MORE](#rip_no_more)). It also explains how each command entry in this reference is documented. The number systems ([MegaNums](../../GLOSSARY.md), [UltraNums](../../GLOSSARY.md), [Base Math](../../GLOSSARY.md)) used by command parameters are in [1.2 Math & Coordinates](1.2-math-and-coordinates.md); file-authoring conventions (script containers, CRLF conventions, the `.RIP`/`.FN`/`.COL` file family) are in [6.0 Writing 3.x .RIP Files](6.0-writing-rip-files.md). The 3.x syntax is the 2.30 syntax verbatim — the inherited rules below are the published 2.00a4 language definition — overlaid here with the byte-exact behavior of the shipping 3.0.7 driver (observed through the RIPtel corpus) and SyncTERM's 3.0 parser.
...
> **Editor's note — the 2.30 product era:** every shipping 2.30-generation RIPterm binary — Professional 2.0, 2.20.01, and 2.30 — carried the same literal auto-sense response string `RIPSCRIP020000`: version 02.00 with the trailing vendor/sub-version digits zeroed. The reported language level never advanced past 2.00.00 through the end of the RIPterm line in October 1997, even as the product versions did; the jump to `RIPSCRIP03000…` arrives with the RIPtel-era 3.0 driver documented here (see [1.0 Introduction](1.0-introduction.md#the-30-identity)).
