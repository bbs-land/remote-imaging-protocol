...
With mouse regions, buttons, hotkeys and query expressions, RIPscrip gives the host far more than a way to draw pictures: it gives it an **action language**. The RIPscrip 3.0 White Paper names this layer the **Host Command Language (HCL)** and divides it into five areas — control characters, pop-up picklists, local playback directives, template directives, and text variables. A [host command](../../GLOSSARY.md) is, at its simplest, raw text transmitted to the host when a button or mouse field is clicked; every HCL directive is something the terminal interprets _before_ (or instead of) transmitting. The 2.30 record this generation inherits is [5.0 of the v2.30 reference](../../2.30/ripscrip/5.0-host-commands.md) (1.54 lineage in the [v1.54 page](../../1.54/ripscrip/5.0-host-commands.md)).

_Evidence: WP; 2.00a4._

This reference splits the HCL across the chapter-5 pages:

- **This page** — where host commands run, control characters, query deferral, mouse-field identities, reading/writing user variables from a command string, web-URL launching, buffer-preservation care, and the host-defined screen-refresh sequence.
- [5.1 Text Variables](5.1-text-variables.md) — the full inventory of pre-defined and user-defined `$…$` variables, including persistence.
- [5.2 Templates & Conditionals](5.2-templates-and-conditionals.md) — the `[N:]` template system and the 3.x `<<IF>>` / `<<NAME>>` macro layer.
- [5.3 Local File Playback](5.3-local-playback.md) — the `$>` / `$)` / `$<` / `$(` playback directives.
- [5.4 Pop-Up Lists](5.4-popup-lists.md) — the `((…))` picklists.
- [3.1 Column Text System](3.1-column-text-system.md) — the flowed-text/overflow machinery the HCL pagination variables drive.
...
In the original 2.x chart, the first three rows (Button Host Commands, Simple Mouse Fields, Query Command) are grouped as "Host Related Commands" and the last three rows (Button Labels, RIP_TEXT_XY, RIP_TEXT) are grouped as "Text Output Related Commands". _Evidence: 2.00a4._
...
The full reference syntax for a user-defined [text variable](../../GLOSSARY.md) inside a host command is unchanged in shape from 2.30, and RIPTEL.HLP documents it completely:
...
The `&` directive was defined in 2.30 as _transparent retrieval to the host_. The 3.x corpus shows its far more powerful role: **inline interpolation into command arguments**. A `$&NAME$` expands to the variable's value wherever it appears — including inside the argument block of drawing commands:
...
_Evidence: 2.00a4 ([v2.30 entry](../../2.30/ripscrip/5.0-host-commands.md#rip_set_refresh)); HLP (`RIP_SetRefresh`; `$REFRESH$`/`$NOREFRESH$` variables, "refresh expressions", DLL export `RIP_RefreshSend`) — not observed in the RIPtel demo corpus, not in SyncTERM._
