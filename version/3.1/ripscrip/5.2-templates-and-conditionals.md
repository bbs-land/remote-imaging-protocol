...
Templates are the Host Command Language's mechanism for _building_ [host commands](5.0-host-commands.md) out of pieces — the White Paper calls them "a cookie-cutter approach to host commands," constructed from whichever buttons are selected and which are not. The 2.30 template system carries into 3.x intact; **layered on top of it, the 3.x era adds something no published specification describes: a genuine macro and conditional layer** (`<<NAME>>`, `<<IF>>…<<ELSE>>…<<ENDIF>>`) evaluated before command execution — see [The 3.x Macro Layer](#the-3x-macro-layer--name) and [Inline Conditionals](#inline-conditionals--if--elseif--else--endif) below. This page covers both: the inherited template system first (merged here from [5.2 of the v2.30 reference](../../2.30/ripscrip/5.2-templates.md); 1.54 lineage in the [v1.54 page](../../1.54/ripscrip/5.2-host-command-templates.md)), then the 3.x additions. Which host-command features may appear where is summarized in [5.0 Host Commands](5.0-host-commands.md#what-can-go-where).
