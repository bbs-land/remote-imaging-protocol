...
This page covers speaker tone generation — waveform-style sounds produced by the terminal itself, triggered through Active [Text Variables](5.1-text-variables.md) anywhere text variables are allowed. This is the 1.54 sound capability ([4.3 in the v1.54 reference](../../1.54/ripscrip/4.3-tone-and-sound-generation.md)) carried forward through 2.30 ([4.4 in the v2.30 reference](../../2.30/ripscrip/4.4-tone-and-sound-generation.md)) into the 3.0 engine: the canned effects take parameters (`$BEEP(freq,ms)$`, `$MUSIC(count)$`, `$PHASER(start,stop,inc,time)$`, …), and the chainable [`$T$`](#t) tone variable _(v2.A4)_ plays a single tone with **no trailing pause**, so multiple `$T$` variables strung together produce continuous multi-frequency sequences — simple music. Each variable's definition includes the exact `sound()`/`delay()` recipe so every terminal produces the same effect. The 3.0.7 driver demonstrably contains this tone-generation family: its error strings validate tone commands with **frequency, duration, and increment sweep** parameters ("start frequency greater than stop", zero increment/duration errors), and RIPTEL.HLP's variable tables document the named sounds; `$RESET(SOUND)$` resets the sound subsystem ([5.1](5.1-text-variables.md#reset)). No wire-level opcode for direct tone commands has been recovered from the corpus _(the demos ship no audio at all)_. All sound variables are _actions_: they expand to an empty string and play a PC-speaker effect. A terminal that does not implement a particular sound should still recognize the variable and do nothing (see the implementer note in [5.1](5.1-text-variables.md)). Digitized audio **files** (`.WAV`) are a separate capability — see [4.5 Audio Files](4.5-audio-files.md) for the [RIP_PLAY_AUDIO](4.5-audio-files.md#rip_play_audio) command and [5.3 Local File Playback](5.3-local-playback.md) for the `$)FILE.WAV$` host-command trigger; neither has anything to do with the speaker tones on this page. The summary table of all variables is in [9.1 Text Variable Reference](9.1-text-variable-reference.md).
