...
This page covers color in RIPscrip 3.x as it shipped in RIPtel: the **256-entry Drawing Palette** (color lookup table) the 2.30 engine introduced, the two color modes — palette mapping and direct RGB encoding — and the palette commands old and new. It also carries the [write mode](../../GLOSSARY.md) command, whose raster operations (COPY/XOR/OR/AND/NOT) apply to **all** primitives, not just simple lines as in 1.54. Colors marked `:CM` in command entries follow the current color mode set by [RIP_SET_COLOR_MODE](#rip_set_color_mode); direct-RGB values are always four-digit [UltraNums](../../GLOSSARY.md) regardless of the [Base Math](../../GLOSSARY.md) setting. Up to 36 complete palettes live in the color palette [data table](7.1-data-tables.md), switched with [RIP_SWITCH_PALETTE](#rip_switch_palette). The 16-color 1.54 model this extends is [2.0 in the v1.54 reference](../../1.54/ripscrip/2.0-colors-and-attributes.md); the published 2.x draft record is [2.0 in the v2.30 reference](../../2.30/ripscrip/2.0-color-modes-and-palettes.md); renderer-side palette/DAC scaling details are in the [2.30 canvas, palette & RGB techspec](../../2.30/techspecs/2.0-canvas-palette-rgb.md). In the RIPtel demo corpus, the standard scene prologue `!|J10|n2000|M08|fZKQO` puts every scene into palette-mapping mode with 8-bit components (`M08`) before drawing begins.
...
**Attributes used:** Draw Color, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_color)) · HLP (`RIP_Color`) · corpus (359 uses in 49 files) · SyncTERM (ripper.c:14366)
...
**Attributes used:** Back Color, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_back_color)) · HLP (`RIP_BackColor`) · corpus (229 uses in 41 files) — not in SyncTERM
...
**Attributes used:** Draw Color, Back Color, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_set_palette)) · HLP (`RIP_SetPalette`) · SyncTERM (ripper.c:13921) — not observed in the RIPtel demo corpus
...
**Attributes used:** Draw Color, Back Color, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_one_palette)) · HLP (`RIP_OnePalette`) · corpus (7 uses in LANDSCPE.RIP, LGF1.RIP) · SyncTERM (ripper.c:14221)
...
**Attributes used:** Draw Color, Back Color, Base Math (current setting; color values always UltraNums) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_set_drawing_palette)) · HLP (`RIP_SetDrawingPalette`) · corpus (BLUEFADE.FN, single use — the 256-color faded background) · SyncTERM (ripper.c:9173 — descriptor-only `2#4#2#`, no handler)
...
**Attributes used:** Draw Color, Back Color, Base Math (current setting; color value always UltraNums) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_one_drawing_palette)) · HLP (`RIP_OneDrawingPalette`) · corpus (65 uses in TUNNEL.RIP) — not in SyncTERM
...
**Attributes used:** Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_set_color_mode)) · HLP (`RIP_SetColorMode`, plus `$COLORMODE$` variable) · corpus (94 uses in 90 files) — not in SyncTERM

This command determines whether Color Mapping mode is in use, or if Direct RGB Colors are being used for color codes. See [Palette Mapping and Direct RGB Mode](#palette-mapping-and-direct-rgb-mode) above for a detailed explanation of the differences between these modes.
...
Every corpus prologue sends `!|M08` — mode 0 (color map) with 8 bits — annotated in TELPORT.FN as "Set color palette mapping mode (not RGB encoding)". HLP error strings confirm the 3.0 driver's Direct RGB support is 8 bits per channel only ("RGB color mode only supports 8-bit color currently").
...
**Attributes used:** Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_switch_palette)) · HLP (`RIP_SwitchPalette`; "Can't protect color palette zero", palette animation strings, `$SCP$`/`$RCP$` variables) — not observed in the RIPtel demo corpus, not in SyncTERM
...
**Attributes used:** Write Mode, Base Math (current setting) **Evidence:** 2.00a4 ([2.30 entry](../../2.30/ripscrip/2.0-color-modes-and-palettes.md#rip_write_mode)) · HLP (`RIP_WriteMode`) · corpus (44 uses in 35 files) · SyncTERM (ripper.c:14062)
