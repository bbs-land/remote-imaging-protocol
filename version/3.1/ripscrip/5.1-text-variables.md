...
This page covers the 3.x [text variable](../../GLOSSARY.md) system — the 2.30 engine carried forward intact, plus the 3.x-era additions recovered from RIPtel. Text variables are `$NAME$` sequences usable in [host commands](5.0-host-commands.md), [queries](5.5-file-transfer-and-queries.md#rip_query), button commands and keystroke macros; **data** variables substitute a value, **active** variables perform an action on the terminal. The 3.x inventory is known from three overlapping sources: the RIPtel 3.1 help files document roughly 60 data-returning and 60 command-performing variables (`HLP`); the demo corpus shows **269 distinct `$…$` forms** actually used in shipping scripts (`corpus`); and SyncTERM's open-source implementation carries a **103-entry built-in table** (`SyncTERM`). Where the sources disagree, the divergence is called out on the entry. Sound-generation variables are documented in [4.4 Tone & Sound Generation](4.4-tone-and-sound-generation.md); the playback prefixes (`$>`, `$<`, `$(`, `$)`) are in [5.3 Local File Playback](5.3-local-playback.md); the template forms (`$?$`, `$?x$`) and the `<<IF>>` conditional layer that consumes variable values are in [5.2 Templates & Conditionals](5.2-templates-and-conditionals.md). The consolidated table of every variable is [9.1 Text Variable Reference](9.1-text-variable-reference.md).
...
_Evidence: HLP (RIPtel's variable-database strings — see the [resource-container research notes](../../3.1/research/riptel-resource-containers.md)); SyncTERM (ripper.c:7899–7961, 19008)._
