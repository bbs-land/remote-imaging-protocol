...
Implementer documentation, numbered in implementer learning order: parse the stream → render correctly → decode the content files → store the assets. Language semantics are not repeated here — see the companion [language reference](../ripscrip/README.md); shared terminology is defined in the [glossary](../../GLOSSARY.md). Formats are documented in the earliest version where they appear (see [CONTRIBUTING.md](../../../CONTRIBUTING.md#technical-specifications-techspecs)); the [2.30](../../2.30/techspecs/README.md) and [3.1](../../3.1/techspecs/README.md) techspecs document only deltas and link back to these pages.
...
- Reference implementations: `sbbs:src/syncterm/ripper.c` (SyncTERM), `RIPtermJS:src/BGI.js` / `src/ripterm.js`, `icy_tools:crates/icy_parser_core/src/rip/`, `pablodraw:Source/Pablo/Formats/Rip/` — catalogued in [CONTRIBUTING.md](../../../CONTRIBUTING.md#reference-repositories-srcrip-tools)
