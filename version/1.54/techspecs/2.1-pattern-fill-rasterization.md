...
Where fill patterns are anchored, how line patterns advance along a figure, and what flood fill actually touches are all under-specified by the 1.54 spec. This page reconstructs the pattern and fill rasterization rules from the reference implementations — chiefly SyncTERM's `ripper.c`, whose code is annotated with behaviors "verified against RIPterm" (`sbbs:src/syncterm/ripper.c`), and RIPtermJS (`RIPtermJS:src/BGI.js`), a from-scratch BGI reimplementation tested against RIPterm 1.54 output. Where a behavior is one implementation's choice rather than verified RIPterm behavior, it is marked as such. The companion page [Canvas, Clipping & Write Modes](2.0-canvas-clipping-write-modes.md) covers the screen model, clipping, write modes, and reset semantics these rules operate under.
