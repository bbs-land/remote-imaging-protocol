...
The 1.54 specification describes commands but often not their pixel-level semantics: what exactly XOR mode XORs, what happens when an icon crosses the viewport edge, which state a reset actually clears. This page and [Pattern & Fill Rasterization](2.1-pattern-fill-rasterization.md) collect those under-specified behaviors as reconstructed from the reference implementations — chiefly SyncTERM's `ripper.c`, whose code is annotated with behaviors "verified against RIPterm" (`sbbs:src/syncterm/ripper.c`), and RIPtermJS (`RIPtermJS:src/BGI.js`), a from-scratch BGI reimplementation tested against RIPterm 1.54 output. Where a behavior is one implementation's choice rather than verified RIPterm behavior, it is marked as such. This page covers the screen model, viewport clipping, write modes, the image put-mode family, and reset semantics; line/fill pattern rasterization is in [2.1](2.1-pattern-fill-rasterization.md). Renderer policy questions for modern implementations (canvas sizes, aspect ratio) live in [version/IMPLEMENTATION.md](../../IMPLEMENTATION.md), not here.
