...
This page covers the closed shapes — rectangles, bars, polygons — and everything to do with filling: the filled shape variants (filled ovals, pie slices, filled polygons), the fill-style state (twelve stock patterns plus user-defined 8×8 patterns), and flood fill. Filled interiors use the current fill color and fill pattern; shape outlines use the current drawing color and line style from [2.0](2.0-colors-and-attributes.md) and [2.1](2.1-lines.md). Raster-level fill semantics (pattern alignment, flood-fill behavior) are implementer material — see the [pattern & fill rasterization techspec](../techspecs/2.1-pattern-fill-rasterization.md).
