...
This page covers the wire-level shape of the RIPscrip language: the `!|` line form, the `|` delimiter, the [command level](../../GLOSSARY.md) hierarchy, escaping, line continuation, and the ANSI [auto-sense](../../GLOSSARY.md) handshake. It also explains how each command entry in this reference is documented. For the number system used by command parameters see [1.2 Math & Coordinates](1.2-math-and-coordinates.md); for file-authoring conventions (line length, CRLF, CP437) see [6.0 Writing .RIP Files](6.0-writing-rip-files.md).
...
Implementers should note that terminals parse this syntax as a byte stream, not line-by-line — the parser-level details (dispatch on `|`, CR/LF handling, continuation resolution) are covered in the [.RIP stream parsing techspec](../techspecs/1.0-rip-stream-parsing.md).
