...
This page covers the drawing state every graphics primitive depends on: the 16-color RIP palette selected from the master 64-color EGA palette, the current drawing color, and the [write mode](../../GLOSSARY.md) (copy vs XOR) that governs how pixels combine with what is already on screen. Line and fill styling state is covered alongside the primitives that use it, in [2.1 Lines](2.1-lines.md#rip_line_style) and [2.3 Shapes & Fills](2.3-shapes-and-fills.md#rip_fill_style).
