...
This page covers raster imagery in RIPscrip 1.54: capturing screen regions to the terminal's [clipboard](../../GLOSSARY.md) (the "scissors" operation), pasting them back with logical put modes, and saving/loading named [icon](../../GLOSSARY.md) files on the terminal's disk. Icons are the workhorse of RIP scene design — captured once, they can be re-stamped endlessly without re-transmission, sidestepping modem bandwidth. What `.ICN`, `.MSK` and `.HIC` files are for is summarized in [6.1 Content File Roles](6.1-content-file-roles.md); how hosts deliver them is [6.2 Asset Delivery & Storage](6.2-asset-delivery-and-storage.md); the byte-level file layout is in the [icon format techspec](../techspecs/3.0-icon-format.md).
