...
This page covers the text side of the RIPscrip display model: the [text window](../../GLOSSARY.md) — the virtual TTY region that displays all non-RIPscrip (ASCII/ANSI) data — and the commands that define, position within, and erase it. Text-window content is [CP437](../../GLOSSARY.md) text rendered from the client's own fonts; the ANSI/VT-100 subset the era's terminals honored is documented in the baseline [ANSI/VT support reference](../../baseline/techspecs/ansi-vt-support.md). The graphics-side model is in [1.3 World View & Virtual Canvas](1.3-world-view-virtual-canvas.md); the full-reset command shared by both windows is [RIP_RESET_WINDOWS](1.3-world-view-virtual-canvas.md#rip_reset_windows).

Every byte received that is not part of a RIPscrip command is routed here: ANSI color and cursor movement codes, plain text, and user keystrokes echoed by the BBS. The text window may also be hidden entirely, making the display pure graphics.
