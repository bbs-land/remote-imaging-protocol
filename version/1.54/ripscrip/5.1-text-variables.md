...
This page covers [text variables](../../GLOSSARY.md) — `$NAME$` values the terminal substitutes into [host commands](5.0-host-commands.md) — in all three forms: built-in variables that return a value, Active Text Variables that perform an action, and User Text Variables the host defines and queries. The full built-in listing is below; the sound-producing Active Text Variables are detailed in [4.3 Tone & Sound Generation](4.3-tone-and-sound-generation.md), and the consolidated table of every variable is [9.1 Text Variable Reference](9.1-text-variable-reference.md).
