...
This page covers text regions — rectangular portions of the graphics [viewport](../../GLOSSARY.md) that display formatted, optionally justified streams of text, typically used to import a text document into a RIPscrip scene. A region is bracketed by a begin/end command pair with one or more line commands in between. The text draws with the current graphics-text settings from [3.0 Text Output & Fonts](3.0-text-output-and-fonts.md).
