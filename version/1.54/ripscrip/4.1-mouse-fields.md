...
This page covers [mouse fields](../../GLOSSARY.md) — host-defined rectangular hot regions that transmit a [host command](../../GLOSSARY.md) back to the host when clicked — and the command that deletes them. Mouse fields are the simplest interactive object in RIPscrip; styled [buttons](4.2-buttons.md) build on the same click-to-host-command machinery. What the transmitted text can contain (control characters, text variables, pick lists, templates) is chapter 5's subject, starting at [5.0 Host Commands & Control Characters](5.0-host-commands.md).

Two global rules apply to all mouse regions (fields and buttons alike): _(v1.54)_

- You are limited to a maximum of 128 Mouse Regions or Mouse Buttons combined — 128 mouse fields, 128 mouse buttons, or any combination, but their total number cannot exceed 128.
- When the user clicks the mouse on the screen, all mouse regions (whether mouse fields or mouse buttons) are scanned from most recent to the least recent ("last in, first out"). If a mouse region overlaps another, the most recently received one is the one clicked.
