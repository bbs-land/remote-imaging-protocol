...
This page covers the sound capability of 1.5x: waveform-style PC-speaker tone sequences triggered through Active [Text Variables](../../GLOSSARY.md). There are no audio **files** in RIPscrip 1.54 — file-based audio (`.WAV`, MIDI) begins with the 2.x generation. Each sound variable below is a defined frequency/delay sequence the host can trigger anywhere text variables are allowed (host commands, queries, keystroke macros); the specification gives the exact `sound()`/`delay()` recipes so every terminal produces the same effect. The text-variable mechanism itself is covered in [5.1 Text Variables](5.1-text-variables.md); the consolidated variable listing is [9.1 Text Variable Reference](9.1-text-variable-reference.md).

A terminal that does not implement a particular sound SHOULD still recognize the variable and do nothing, rather than prompting the user for a value (see the compatibility note in [5.1 Text Variables](5.1-text-variables.md#compatibility-note-for-terminal-implementers)).
