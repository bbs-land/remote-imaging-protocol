...
This page covers the remaining level-1 (and one level-9) commands: defining and querying [text variables](5.1-text-variables.md) from the wire, copying screen regions, playing back stored scenes, querying files on the terminal's disk, entering block-transfer mode, and ending a scene. The conceptual treatment of variable definition/querying is in [5.1 Text Variables](5.1-text-variables.md#user-text-variables); the asset-delivery workflow built from RIP_FILE_QUERY and RIP_ENTER_BLOCK_MODE is in [6.2 Asset Delivery & Storage](6.2-asset-delivery-and-storage.md).
