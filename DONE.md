...
- [x] Convert RIPscrip 2.00 alpha 4 specification to linked Markdown (`version/2.0/ripscrip/`, 22 files)
- [x] Convert the RIPscrip 3.0 technical white paper to linked Markdown (`version/3.0/ripscrip/` files 01-08), plus an editorial reconstructed reference (files 09-11) from SyncTERM `ripper.c` evidence with per-claim citations
...
**2.0 fidelity audit + ERRATA restructure (2026-08-10).** Fourteen read-only audits covered all 43 pages of `version/2.0/{ripscrip,techspecs}/`, then three opus reviewers adversarially re-verified every defect **and its proposed fix** before anything was edited.

**Why this differed from the 1.54 audit:** the canonical text is a **2.00 ALPHA 4 draft that predates the shipping release**, so material beyond it is legitimate - portions were reconstructed from usage, inference and post-draft errata. Absence from the draft was explicitly not treated as a defect. Two things made verification unusually tractable: the draft carries **10,088 change bars** (`> v2.A0`..`> v2.A4`) against our 536 `_(v2.Ax)_` tags, and three distinct shipping binaries survive (2.0, v2.20.01, 2.30).

**Result: zero forward-ported 3.x features anywhere in the tree.** Inventories are exact bidirectional matches - 9.0 = 95/95 commands, 5.1 = 132/132 variables. 33 defects were consolidated; the review gate returned **18 CONFIRMED, 11 CONFIRMED-REVISED, 4 REFUTED**, and found 1 defect the audits missed.

**The gate earned its place - four items would have damaged the docs if applied as first reported:**

- A "`b`/`J` corpus count is wrong" defect was **manufactured during consolidation**; "90 of 116" was correct, and the "91" came from grepping a bare `J10` token that also matches a RIP_MOUSE coordinate string.
- The **entire systemic-tagging cluster was refuted** by a 517-tag mechanical sweep: the pages' real convention is the **modal (bulk) change bar** (83.8% conformance), not "latest-bar-wins" (48.5%, the worst-fitting rule). Enforcing the proposed rule would have rewritten 8 correct tags into wrong ones. True violations: 10 paragraphs, 1 unambiguous.
- A pointer-table fix was **backwards** - the quoted values `0x240/0x248/0x250` were right; the wrong token was the offset (`0x122` should be `0x11C`).
- An auto-sense retag would have made the page worse: 10 of 11 draft lines are v2.A3, so the correct tag is `_(v2.A3, revised v2.A4)_`, not `_(v2.A4)_`.

**Defects fixed** included the RIPterm 2.30 binary date (October 1997, not November 1995 - the 1995-11 files are carried-forward 2.20.01-vintage support files; `CONTRIBUTING.md` had it inverted too), a **fabricated claim that 1.54 restricted write modes to simple lines** (present in both 2.0 and 3.0; 1.54's matrix gives Write Mode YES to text, rectangles, polygons, poly-lines and beziers), three Format-line command bugs inherited from the draft's copy-paste errors (`!|z`->`!|x` and `!|t`, `!|2s`->`!|2p` - all three already fixed in 3.0 but never back-ported), the `RIP_SET_DRAWING_PALETTE` argument order, corrected counts (`.BMH` 22 not 24, icons 164 not 158, WAV data chunk 74,990 not 75,006, 13 commands/line not 10, 14 footer files not 17), four bad citations, an internal WAV-routing contradiction between two techspec pages, and CONTRIBUTING's structurally broken artifact provenance table (orphaned rows, two rows crammed on one line, "251 files" -> 253, plus the two undocumented reference installs).

**RIP_FILL note corrected on owner input.** The claim that clients "parse and ignore" flood fill was wrong. The real history: it was unreliable in the actual clients and authoring convention moved away from it. This is now citable rather than recollected - the draft states its own rationale ("having a reliable fill operation cannot be achieved under multiple resolutions and platforms") and SyncTERM's implementation is literally named `broken_flood_fill` (`ripper.c:13593`), with a 492-entry seed stack that silently drops pushes. Corpus measurement: 3,787 uses across 78 of 118 1.5x files, zero thereafter - so the note deliberately does **not** claim 1.5x artists avoided it, nor treat the later zeros as evidence of unreliability (the command was formally removed, so zeros are expected regardless).

**ERRATA restructure (owner decision).** Per the principle that "the main document should be what is expected in a working client implementation", `version/2.0/ripscrip/ERRATA.md` was created and **all 16 inline editor's notes migrated out of the chapter pages**, which now carry as-shipped truth without qualification. Building ERRATA first, read-only, exposed three pages whose bodies still held the draft's wrong values while an inline note made them _look_ reconciled - `RIP_SET_BASE_MATH` still tabled `b` (shipped opcode is `J`), `RIP_EXTENDED_FONT_STYLE` still showed the never-shipped 13-character layout and fictional font names, and `4.1` said JPEGs delete on "flag 4 or 8" while its own table three lines below says flag 2. The extended-font entry was rebuilt from 2.30-specific evidence (18 `!|y` uses in the shipping install, six fields attested by 2.30's own `RIPTERM.HLP` error strings, `.RFF` fonts not 2.0's `.FF1`), with the one unprovable sub-field marked as unconfirmed rather than invented. **2.0 only** - 1.54 shipped a final spec and 3.0 is a reconstruction whose evidence tags are integral, so both keep their inline notes.

Link checker clean at 0 bad of 3,894 links across 149 files; Prettier clean; `version/*/text/` untouched throughout.
...
**Verbatim `text/` documents (owner edits, 2026-08-10):** the TeleGrafix contact block was redacted from `version/2.0/text/RIPScrip-2.0-alpha-4.txt` (13 lines), and `version/1.54/text/RIPScrip-1.54.txt` was re-encoded to valid UTF-8 - it was the last document still holding raw CP437 high bytes (30 lines of box-drawing art in the Radio Button template examples, which the CP437→UTF-8 normalization in CONTRIBUTING.md had missed). All three `text/` documents are now valid UTF-8, closing the hazard where a UTF-8 editor would silently replace those bytes with U+FFFD.
...
**Prelude/introduction trim pass (2026-08-10).** Every page under `version/{1.54,2.0,3.0,baseline}/{ripscrip,techspecs}/` opened with an "on this page" prelude - the block between the top nav line and the first `##` heading. Across 122 pages these totalled ~18,000 words, many of them 150-360-word single paragraphs restating the page before the reader reached it. Trimmed to **11,755 words (-35%)**; 116 files changed, 451 insertions / 218 deletions, link checker clean throughout.

Three prelude forms, chosen per page: a **one-sentence scope line** (default, subject stated directly - no "This page covers…", of which zero remain); **scope line + category anchor list** for long pages; **scope line + one italic pointer line** where cross-version lineage needed to stay discoverable. Depth varies deliberately - pages whose "prelude" is actually original TeleGrafix prose (`1.54/5.2`, `1.54/5.3`) were barely touched, and six pages were verified as needing **no change at all** (`1.54/9.2`, `2.0/9.2`, `3.0/9.2`, the `2.0/techspecs` README and its `4.0` delta, `2.0/techspecs/3.5`).
...
- `2.0/2.0` and `3.0/2.0` claimed direct-RGB values are "always four-digit UltraNums", contradicting RIP_COLOR's own 1-4-digit table on the same pages. Dropped.
- `1.54/9.3`, `2.0/9.3` and `3.0/9.3` scoped themselves as "everything that flows terminal → host", omitting the `## Host-to-terminal ANSI control` section (and, on 3.0, mislabelling its direction). Rewritten to name both directions.
...
- **Pinned-release directories** - `version/{1.54,2.0,3.0}/`, one directory per generation (originally named for each generation's definitive release, later renamed to the RIPscrip language version); the `version-transition/` staging area is retired.
...
- [x] **Promotion into `version/`** (2026-08-09): the six staged trees replaced the old editions as `version/{1.54,2.0,3.0}/{ripscrip,techspecs}/` (dirs renamed from `1.5x`/`2.x`/`3.x`), the glossary landed at `version/GLOSSARY.md`, and `version-transition/` was retired. Repo-wide link rewrite: 502 broken links repaired in three passes - renamed path segments and collapsed `../../../version/…` depth (96 files), 174 links retargeted from pre-restructure page names by resolving each anchor to the numbered page that now carries the heading (so e.g. `#rip_button` lands on `4.3-buttons.md`, not a generic index), then the version-root docs, three orphaned anchors and one code-span path by hand. The Markdown white-paper edition was retired in the same pass (the verbatim `3.0/text/` document remains and all references point there). Verified 0 bad of 3,862 links across 147 files, Prettier clean
- [x] **Version READMEs finalized** (2026-08-09): `version/README.md` rewritten to carry the structure and organizing principles (layout tree, numbering, audience split, self-contained vs delta, format-first techspecs, glossary), version table relabelled 1.54/2.0/3.0, root `README.md` layout block corrected; each tree README gained its source-document identity and provenance carried over from the old editions (spec revision and date, definitive release, record status), and the techspecs READMEs gained **Primary sources** sections
- [x] Restructure the 3.x docs to mirror the 2.x layout: `version/3.0/ripscrip/` is now a 22-file reconstructed protocol reference with per-claim evidence tags; the white-paper conversion moved to `version/3.0/whitepaper/` _(that Markdown edition was later retired in the promotion pass - only the verbatim text under `version/3.0/text/` remains)_
...
- [x] Mine SyncTERM's `ripper.c` (claims "RIP 3.0 compatible") for the actual 3.0 command set, syntax changes, and behaviors; document with citations (→ `version/3.0/ripscrip/09-11`)
...
- [x] 3.x techspecs written as deltas (2026-08-08, `version/3.0/techspecs/`): `rff-additions.md` (BRUSH/EUREKA/OAKLAND decoded; upper-case style suffixes prove a separate converter run; no copyright trailer; BRUSH's >64 KiB trailer confirms the u16→u32 widening motive), `maf-fonts.md` (RIPscrip.maf: 6 resolutions incl. windowed-size sets, third glyph-art revision). The RES/DB/HLP container decodes were relocated to `version/3.0/research/riptel-resource-containers.md` (client packaging, not spec surface). JPEG is documented at its earliest appearance in `version/2.0/techspecs/jpeg-images.md`; MIDI/sequenced music confirmed to have never materialized in any spec or product
...
- Dropped after discussion (2026-08-08): decoding the `RIPTERM.RES`/`RIPSCRIP.RES` container directories - a detail of the prior software's packaging, not part of the specification surface an alternative implementation needs. `RIPSCRIP.DB` likewise abstracted out of the spec docs: persistence is documented as a plain key/value store (name ≤ 20 chars → value ≤ 255 chars); the byte decodes live in `version/3.0/research/riptel-resource-containers.md`
...
- [x] Populate `version/2.0/assets/` - done 2026-08-08 from the two-disk RIPterm Professional 2.0 install (`~/src/rip-tools/RIPTerm2.0/extracted/`): complete `FONTS\` (10 `.CHR`, 5 Atech `.FF1`, per-resolution `.FNT` + widget `.IMG`s, `DEMO_ONE.EXE`), `ICONS\` (91 `.BMP`, 3 `.BMH`, `SHADOW.RIP`), `RIPTERM.FNT`, and `RIPTERM.WAV`, with per-file READMEs
- [x] Recover the 2.2/2.3-era assets - complete RIPterm 2.30 shareware distribution recovered 2026-08-08 (VOGONS; `artifacts/ripterm-2.30/RIPT2300.zip`, full 251-file set unpacked to `artifacts/ripterm-2.30/extracted/` incl. `FONTS\` and `ICONS\`); deltas vs the 2.0 set staged into `version/2.0/assets/` and the asset READMEs extended (2026-08-08 interrogation)
...
Complete: 1.5x and 2.x Markdown reference editions; 3.x edition rebuilt to mirror the 2.x layout (22-page reconstructed reference with evidence tags, whitepaper conversion at `version/3.0/whitepaper/`, research notes at `version/3.0/research/`); RIPtel 3.1 fully analyzed (script census, help-file extraction, binary formats); assets staged for 1.5x and 3.x with per-file READMEs; `temp/syncterm-missing-feature-rip3.md` gap checklist; Git LFS + `.gitattributes` (LF default, `.rip`/`.ans` and `assets/**` byte-exact); reference repos + binaries in `~/src/rip-tools/` (see CONTRIBUTING); `tools/check-links.py` validates all doc links. Rendering guidance in `version/IMPLEMENTATION.md`.
